...
TEST(IntegrationTest, ErrorCallback) {
    ...
    t1->set_cpu_function([](TaskContext &ctx) { throw std::runtime_error("Intentional failure"); });
...
TEST(IntegrationTest, LargeGraph) {
    ...
    for (int i = 0; i < NUM_TASKS; ++i) {
        ...
        task->set_cpu_function([&](TaskContext &ctx) { ++completed_count; });
...
TEST(IntegrationTest, Version) {
    ...
    EXPECT_EQ(hts::VERSION_MINOR, 2);
    EXPECT_EQ(hts::get_version(), "1.2.0");
}
