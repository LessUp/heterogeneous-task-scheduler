...
class ExecutionEngineTest : public ::testing::Test {
  protected:
    void SetUp() override {
        ...
        engine_ = std::make_unique<ExecutionEngine>(*pool_, 4, 4); // 4 CPU threads, 4 GPU streams
        task_id_counter_ = 0;
    }
    ...
    TaskId next_task_id() { return task_id_counter_++; }
...
TEST_F(ExecutionEngineTest, CPUTaskSubmission) {
    auto task = std::make_shared<Task>(next_task_id(), DeviceType::CPU);
...
TEST_F(ExecutionEngineTest, GPUTaskSubmission) {
    auto task = std::make_shared<Task>(next_task_id(), DeviceType::GPU);
    std::atomic<bool> executed{false};
...
TEST_F(ExecutionEngineTest, TaskExceptionHandling) {
    ...
    task->set_cpu_function([](TaskContext &ctx) { throw std::runtime_error("Test exception"); });
...
TEST_F(ExecutionEngineTest, ConcurrentCPUAndGPUTasks) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        task->set_gpu_function(
            [&gpu_counter](TaskContext &ctx, cudaStream_t stream) { gpu_counter++; });
...
TEST_F(ExecutionEngineTest, TaskContextAvailability) {
    ...
    task->set_cpu_function(
        [&has_input_access](TaskContext &ctx) { has_input_access = !ctx.has_error(); });

    auto future = engine_->execute_task(task, DeviceType::CPU);
    future.wait();

    EXPECT_TRUE(has_input_access);
}
