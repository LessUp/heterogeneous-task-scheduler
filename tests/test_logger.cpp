...
TEST(LoggerTest, LevelFiltering) {
  ...
  logger.set_callback(
      [&](LogLevel level, const std::string &msg) { logs.push_back(msg); });
...
TEST(LoggerTest, FormatString) {
  ...
  logger.set_callback(
      [&](LogLevel level, const std::string &msg) { captured = msg; });
...
TEST(LoggerTest, Macros) {
  ...
  logger.set_callback(
      [&](LogLevel level, const std::string &msg) { levels.push_back(level); });
