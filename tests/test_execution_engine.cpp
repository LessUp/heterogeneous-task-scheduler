...
TEST_F(ExecutionEngineTest, CPUTaskSubmission) {
    ...
    task->set_cpu_function([&executed](TaskContext& ctx) {
        executed = true;
    });
...
TEST_F(ExecutionEngineTest, GPUTaskSubmission) {
    ...
    task->set_gpu_function([&executed](TaskContext& ctx, cudaStream_t stream) {
        executed = true;
    });
...
TEST_F(ExecutionEngineTest, ConcurrentCPUAndGPUTasks) {
    ...
    for (int i = 0; i < 5; ++i) {
        auto task = std::make_shared<Task>(DeviceType::CPU);
        task->set_cpu_function([&cpu_counter](TaskContext& ctx) {
            cpu_counter++;
        });
        futures.push_back(engine_->execute_task(task, DeviceType::CPU));
    }
    ...
    for (int i = 0; i < 5; ++i) {
        auto task = std::make_shared<Task>(DeviceType::GPU);
        task->set_gpu_function([&gpu_counter](TaskContext& ctx, cudaStream_t stream) {
            gpu_counter++;
        });
