...
TEST(TaskBuilderTest, BasicBuild) {
    ...
    auto task =
        builder.name("Test Task").device(DeviceType::CPU).cpu([](TaskContext &ctx) {}).build();
...
TEST(TaskBuilderTest, WithPriority) {
    ...
    auto task = builder.priority(TaskPriority::Critical).cpu([](TaskContext &ctx) {}).build();
...
TEST(TaskBuilderTest, WithDependencies) {
    ...
    auto task2 = builder.name("Task 2").after(task1).cpu([](TaskContext &ctx) {}).build();
...
TEST(TaskBuilderTest, MultipleDependencies) {
    ...
    auto task3 = builder.after(task1).after(task2).cpu([](TaskContext &ctx) {}).build();
...
TEST(TaskBuilderTest, ResetAfterBuild) {
    ...
    auto task1 =
        builder.name("Task 1").priority(TaskPriority::High).cpu([](TaskContext &ctx) {}).build();
