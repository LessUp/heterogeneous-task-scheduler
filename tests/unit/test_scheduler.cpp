...
TEST_F(SchedulerTest, SingleGPUTask) {
    ...
    task->set_gpu_function([&executed](TaskContext &ctx, cudaStream_t stream) { executed = true; });
...
TEST_F(SchedulerTest, ErrorCallback) {
    ...
    auto task = scheduler_->graph().add_task(DeviceType::CPU);
    task->set_cpu_function([](TaskContext &ctx) { throw std::runtime_error("Test error"); });
...
TEST_F(SchedulerTest, FailurePropagation) {
    ...
    t1->set_cpu_function([](TaskContext &ctx) { throw std::runtime_error("Failure"); });
...
TEST_F(SchedulerTest, ExecutionStats) {
    ...
    task->set_cpu_function(
        [](TaskContext &ctx) { std::this_thread::sleep_for(std::chrono::milliseconds(10)); });
...
TEST_F(SchedulerTest, DiamondPattern) {
    ...
    t4->set_cpu_function([&](TaskContext &ctx) { t4_value = t2_value + t3_value; });
...
TEST_F(SchedulerTest, PolicySetting) {
    ...
    scheduler_->set_policy(std::make_unique<GpuFirstPolicy>());
    EXPECT_STREQ(scheduler_->policy_name(), "GPU-First");

    scheduler_->set_policy(std::make_unique<CpuFirstPolicy>());
    EXPECT_STREQ(scheduler_->policy_name(), "CPU-First");
