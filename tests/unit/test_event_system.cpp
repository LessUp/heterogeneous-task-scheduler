...
TEST_F(EventSystemTest, SubscribeAndPublish) {
    ...
    events.subscribe(EventType::TaskStarted, [&count](const Event &) { ++count; });
...
TEST_F(EventSystemTest, Unsubscribe) {
    ...
    auto id = events.subscribe(EventType::TaskStarted, [&count](const Event &) { ++count; });
...
TEST_F(EventSystemTest, MultipleHandlers) {
    ...
    events.subscribe(EventType::TaskCompleted, [&count1](const Event &) { ++count1; });

    events.subscribe(EventType::TaskCompleted, [&count2](const Event &) { ++count2; });
...
TEST_F(EventSystemTest, EventTypeName) {
    EXPECT_STREQ(EventSystem::event_type_name(EventType::TaskCreated), "TaskCreated");
    EXPECT_STREQ(EventSystem::event_type_name(EventType::TaskStarted), "TaskStarted");
    EXPECT_STREQ(EventSystem::event_type_name(EventType::TaskCompleted), "TaskCompleted");
    EXPECT_STREQ(EventSystem::event_type_name(EventType::TaskFailed), "TaskFailed");
    EXPECT_STREQ(EventSystem::event_type_name(EventType::GraphStarted), "GraphStarted");
    EXPECT_STREQ(EventSystem::event_type_name(EventType::GraphCompleted), "GraphCompleted");
}
...
TEST_F(EventSystemTest, HandlerException) {
    ...
    events.subscribe(EventType::TaskStarted,
                     [](const Event &) { throw std::runtime_error("Handler error"); });
