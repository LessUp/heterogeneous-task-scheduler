...
TEST_F(EventSystemTest, SubscribeAndPublish) {
    ...
    events.subscribe(EventType::TaskStarted, [&count](const Event&) {
        ++count;
    });
...
TEST_F(EventSystemTest, SubscribeAll) {
    ...
    events.subscribe_all([&count](const Event&) {
        ++count;
    });
...
TEST_F(EventSystemTest, Unsubscribe) {
    ...
    auto id = events.subscribe(EventType::TaskStarted, [&count](const Event&) {
        ++count;
    });
...
TEST_F(EventSystemTest, MultipleHandlers) {
    ...
    events.subscribe(EventType::TaskCompleted, [&count1](const Event&) {
        ++count1;
    });
    
    events.subscribe(EventType::TaskCompleted, [&count2](const Event&) {
        ++count2;
    });
