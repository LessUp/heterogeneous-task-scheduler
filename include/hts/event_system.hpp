...
struct Event {
    ...
    Event(EventType t) : type(t), timestamp(std::chrono::high_resolution_clock::now()) {}

    Event(EventType t, TaskId id)
        : type(t), task_id(id), timestamp(std::chrono::high_resolution_clock::now()) {}

    Event(EventType t, TaskId id, DeviceType dev)
        : type(t), task_id(id), device(dev), timestamp(std::chrono::high_resolution_clock::now()) {}

    Event(EventType t, TaskId id, const std::string &msg)
        : type(t), task_id(id), message(msg), timestamp(std::chrono::high_resolution_clock::now()) {
    }
};
...
class EventSystem {
  public:
    ...
    void unsubscribe(SubscriptionId id) {
        ...
        global_handlers_.erase(std::remove_if(global_handlers_.begin(), global_handlers_.end(),
                                              [id](const auto &pair) { return pair.first == id; }),
    ...
    void task_cancelled(TaskId id) { publish(Event{EventType::TaskCancelled, id}); }
...
class ScopedSubscription {
  public:
    ScopedSubscription(EventSystem &system, SubscriptionId id) : system_(system), id_(id) {}
