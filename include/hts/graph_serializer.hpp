...
public:
  ...
  static std::string to_json(const TaskGraph &graph) {
    ...
    for (const auto &[id, task] : tasks) {
      ...
      oss << "      \"priority\": \"" << priority_to_string(task->priority())
          << "\",\n";
      oss << "      \"has_cpu\": "
          << (task->has_cpu_function() ? "true" : "false") << ",\n";
      oss << "      \"has_gpu\": "
          << (task->has_gpu_function() ? "true" : "false") << "\n";
      oss << "    }";
      if (++i < tasks.size())
        oss << ",";
      oss << "\n";
  ...
  static bool save_to_file(const TaskGraph &graph,
  ...
  static std::string to_dot(const TaskGraph &graph) {
    ...
    for (const auto &[id, task] : tasks) {
      ...
      std::string label =
          task->name().empty() ? "Task " + std::to_string(id) : task->name();
  ...
  static bool save_dot_file(const TaskGraph &graph,
                            const std::string &filename) {
    std::ofstream file(filename);
    if (!file.is_open()) {
      return false;
