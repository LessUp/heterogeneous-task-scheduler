...
public:
  ...
  static void set_device(int device_id) {
    HTS_CUDA_CHECK(cudaSetDevice(device_id));
  }
  ...
  static std::string device_info_string(int device_id = -1) {
    ...
    std::string result;
    result +=
        "Device " + std::to_string(info.device_id) + ": " + info.name + "\n";
    result += "  Compute Capability: " +
              std::to_string(info.compute_capability_major) + "." +
              std::to_string(info.compute_capability_minor) + "\n";
    result +=
        "  Total Memory: " + std::to_string(info.total_memory / (1024 * 1024)) +
        " MB\n";
    result +=
        "  Free Memory: " + std::to_string(info.free_memory / (1024 * 1024)) +
        " MB\n";
    result +=
        "  Multiprocessors: " + std::to_string(info.multiprocessor_count) +
        "\n";
    result +=
        "  Max Threads/Block: " + std::to_string(info.max_threads_per_block) +
        "\n";
    result += "  Warp Size: " + std::to_string(info.warp_size) + "\n";
    result += "  Concurrent Kernels: " +
              std::string(info.concurrent_kernels ? "Yes" : "No") + "\n";
...
public:
  explicit ScopedDevice(int device_id)
      : previous_device_(CudaUtils::current_device()) {
...
public:
  explicit PinnedMemory(size_t count) : count_(count), ptr_(nullptr) {
    HTS_CUDA_CHECK(cudaMallocHost(&ptr_, count * sizeof(T)));
  }
...
public:
  explicit DeviceMemory(size_t count) : count_(count), ptr_(nullptr) {
    HTS_CUDA_CHECK(cudaMalloc(&ptr_, count * sizeof(T)));
  }
  ...
  void copy_from_host_async(const T *host_ptr, cudaStream_t stream,
    ...
    if (count == 0)
      count = count_;
    HTS_CUDA_CHECK(cudaMemcpyAsync(ptr_, host_ptr, count * sizeof(T),
                                   cudaMemcpyHostToDevice, stream));
  }

  void copy_to_host_async(T *host_ptr, cudaStream_t stream,
                          size_t count = 0) const {
    if (count == 0)
      count = count_;
    HTS_CUDA_CHECK(cudaMemcpyAsync(host_ptr, ptr_, count * sizeof(T),
