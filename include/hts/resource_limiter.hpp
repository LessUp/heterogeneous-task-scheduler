...
class SemaphoreGuard {
  public:
    explicit SemaphoreGuard(Semaphore &sem) : sem_(sem), acquired_(true) { sem_.acquire(); }
...
class ResourceLimiter {
  public:
    ...
    explicit ResourceLimiter(const Limits &limits)
        : limits_(limits),
          cpu_semaphore_(limits.max_concurrent_cpu_tasks > 0 ? limits.max_concurrent_cpu_tasks
                                                             : 1000000),
          gpu_semaphore_(limits.max_concurrent_gpu_tasks > 0 ? limits.max_concurrent_gpu_tasks
                                                             : 1000000) {}

    ResourceLimiter() : ResourceLimiter(Limits{}) {}

    /// Acquire CPU task slot
    bool acquire_cpu_slot() {
        if (limits_.max_concurrent_cpu_tasks == 0)
            return true;
    ...
    bool acquire_gpu_slot() {
        if (limits_.max_concurrent_gpu_tasks == 0)
            return true;
    ...
    bool try_allocate_gpu_memory(size_t bytes) {
        if (limits_.max_gpu_memory_bytes == 0)
            return true;
    ...
    bool can_add_task() const {
        if (limits_.max_total_tasks == 0)
            return true;
        return total_tasks_.load() < limits_.max_total_tasks;
    }

    /// Increment task count
    void add_task() { ++total_tasks_; }

    /// Decrement task count
    void remove_task() { --total_tasks_; }
    ...
    Stats get_stats() const {
        Stats stats;
        stats.current_cpu_tasks =
            limits_.max_concurrent_cpu_tasks > 0
                ? limits_.max_concurrent_cpu_tasks - cpu_semaphore_.available()
                : 0;
        stats.current_gpu_tasks =
            limits_.max_concurrent_gpu_tasks > 0
                ? limits_.max_concurrent_gpu_tasks - gpu_semaphore_.available()
                : 0;
