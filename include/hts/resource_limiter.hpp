...
public:
  ...
  explicit ResourceLimiter(const Limits &limits = {})
      : limits_(limits), cpu_semaphore_(limits.max_concurrent_cpu_tasks > 0
                                            ? limits.max_concurrent_cpu_tasks
                                            : 1000000),
        gpu_semaphore_(limits.max_concurrent_gpu_tasks > 0
                           ? limits.max_concurrent_gpu_tasks
                           : 1000000) {}
  ...
  void free_gpu_memory(size_t bytes) {
    ...
    std::lock_guard<std::mutex> lock(gpu_memory_mutex_);
    current_gpu_memory_ =
        (bytes > current_gpu_memory_) ? 0 : current_gpu_memory_ - bytes;
  }
...
public:
  ...
  ResourceSlotGuard(ResourceSlotGuard &&other) noexcept
      : limiter_(other.limiter_), device_(other.device_),
