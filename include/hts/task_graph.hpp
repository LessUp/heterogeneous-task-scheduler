...
class TaskGraph {
  ...
  private:
    ...
    std::unordered_map<TaskId, std::vector<TaskId>> adjacency_list_;    // from -> [to]
    std::unordered_map<TaskId, std::vector<TaskId>> reverse_adjacency_; // to -> [from]
    ...
    bool dfs_has_path(TaskId start, TaskId target, std::unordered_set<TaskId> &visited) const;
};
