...
public:
  explicit ExponentialBackoffPolicy(
      ...
      : max_retries_(max_retries), initial_delay_(initial_delay),
        multiplier_(multiplier), max_delay_(max_delay) {}
  ...
  std::chrono::milliseconds get_delay(size_t attempt) const override {
    auto delay_ms = static_cast<long long>(initial_delay_.count() *
                                           std::pow(multiplier_, attempt));
...
public:
  explicit JitteredBackoffPolicy(
      ...
      : max_retries_(max_retries), initial_delay_(initial_delay),
        multiplier_(multiplier), max_delay_(max_delay),
        jitter_factor_(jitter_factor), rng_(std::random_device{}()) {}
  ...
  std::chrono::milliseconds get_delay(size_t attempt) const override {
    auto base_delay = static_cast<double>(initial_delay_.count() *
                                          std::pow(multiplier_, attempt));
...
public:
  ...
  static std::unique_ptr<RetryPolicy>
  exponential(size_t max_retries = 5, std::chrono::milliseconds initial_delay =
                                          std::chrono::milliseconds{100}) {
    return std::make_unique<ExponentialBackoffPolicy>(max_retries,
                                                      initial_delay);
  }

  static std::unique_ptr<RetryPolicy>
  jittered(size_t max_retries = 5,
           std::chrono::milliseconds initial_delay = std::chrono::milliseconds{
               100}) {
