...
class NoRetryPolicy : public RetryPolicy {
  public:
    bool should_retry(size_t, const std::string &) const override { return false; }
...
class FixedRetryPolicy : public RetryPolicy {
  public:
    ...
    std::chrono::milliseconds get_delay(size_t) const override { return delay_; }
...
class ExponentialBackoffPolicy : public RetryPolicy {
  public:
    explicit ExponentialBackoffPolicy(
        ...
        : max_retries_(max_retries), initial_delay_(initial_delay), multiplier_(multiplier),
    ...
    std::chrono::milliseconds get_delay(size_t attempt) const override {
        auto delay_ms =
            static_cast<long long>(initial_delay_.count() * std::pow(multiplier_, attempt));
        return std::chrono::milliseconds(
            std::min(delay_ms, static_cast<long long>(max_delay_.count())));
    }
...
class JitteredBackoffPolicy : public RetryPolicy {
  public:
    explicit JitteredBackoffPolicy(
        ...
        : max_retries_(max_retries), initial_delay_(initial_delay), multiplier_(multiplier),
          max_delay_(max_delay), jitter_factor_(jitter_factor), rng_(std::random_device{}()) {}
    ...
    std::chrono::milliseconds get_delay(size_t attempt) const override {
        auto base_delay =
            static_cast<double>(initial_delay_.count() * std::pow(multiplier_, attempt));
        ...
        return std::chrono::milliseconds{static_cast<long long>(base_delay * jitter)};
    }
...
class ConditionalRetryPolicy : public RetryPolicy {
  public:
    ...
    explicit ConditionalRetryPolicy(std::unique_ptr<RetryPolicy> base_policy,
                                    ErrorPredicate predicate)
        : base_policy_(std::move(base_policy)), predicate_(std::move(predicate)) {}
    ...
    static std::unique_ptr<ConditionalRetryPolicy>
    transient_errors(std::unique_ptr<RetryPolicy> base_policy) {
        return std::make_unique<ConditionalRetryPolicy>(
            std::move(base_policy), [](const std::string &error) {
                ...
                return normalized.find("timeout") != std::string::npos ||
                       normalized.find("temporary") != std::string::npos ||
                       normalized.find("transient") != std::string::npos ||
                       normalized.find("retry") != std::string::npos ||
                       normalized.find("busy") != std::string::npos;
            });
    }
...
class RetryPolicyFactory {
  public:
    static std::unique_ptr<RetryPolicy> no_retry() { return std::make_unique<NoRetryPolicy>(); }

    static std::unique_ptr<RetryPolicy>
    fixed(size_t max_retries = 3,
          std::chrono::milliseconds delay = std::chrono::milliseconds{100}) {
        return std::make_unique<FixedRetryPolicy>(max_retries, delay);
    }

    static std::unique_ptr<RetryPolicy>
    exponential(size_t max_retries = 5,
                std::chrono::milliseconds initial_delay = std::chrono::milliseconds{100}) {
        return std::make_unique<ExponentialBackoffPolicy>(max_retries, initial_delay);
    }

    static std::unique_ptr<RetryPolicy>
    jittered(size_t max_retries = 5,
             std::chrono::milliseconds initial_delay = std::chrono::milliseconds{100}) {
