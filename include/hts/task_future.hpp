...
template <typename T> class TaskFuture {
  public:
    ...
    explicit TaskFuture(std::shared_ptr<Task> task)
        : task_(task), state_(std::make_shared<SharedState>()) {}
    ...
    void wait() const {
        ...
        state_->cv.wait(lock, [this] { return state_->has_value || state_->has_error; });
    }
    ...
    bool wait_for(const std::chrono::duration<Rep, Period> &timeout) const {
        ...
        return state_->cv.wait_for(lock, timeout,
                                   [this] { return state_->has_value || state_->has_error; });
    }
  ...
  public:
    ...
    void wait() const {
        if (!valid())
            return;
        std::unique_lock<std::mutex> lock(state_->mutex);
        state_->cv.wait(lock, [this] { return state_->completed || state_->has_error; });
    }
    ...
    bool wait_for(const std::chrono::duration<Rep, Period> &timeout) const {
        ...
        std::unique_lock<std::mutex> lock(state_->mutex);
        return state_->cv.wait_for(lock, timeout,
                                   [this] { return state_->completed || state_->has_error; });
    }
