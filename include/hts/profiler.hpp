...
public:
    ...
    /// Record task start
    void record_task_start(TaskId id, const std::string& name, DeviceType device) {
        if (!active_) return;
    ...
    Summary generate_summary() const {
        ...
        if (!records_.empty()) {
            summary.avg_task_time = (total_cpu_time + total_gpu_time) / records_.size();
        }
        ...
        if (summary.total_time.count() > 0) {
            ...
            summary.gpu_utilization = static_cast<double>(total_gpu_time.count()) / 
                                      summary.total_time.count();
            summary.parallelism = static_cast<double>(total_cpu_time.count() + total_gpu_time.count()) /
                                  summary.total_time.count();
        }
    ...
    std::string generate_report() const {
        ...
        oss << "  Avg task time:   " << summary.avg_task_time.count() / 1e3 << " us\n";
        oss << "  Max task time:   " << summary.max_task_time.count() / 1e3 << " us\n";
        oss << "  Min task time:   " << summary.min_task_time.count() / 1e3 << " us\n\n";
...
private:
    static const char* state_string(TaskState state) {
        switch (state) {
            case TaskState::Completed: return "OK";
            case TaskState::Failed: return "FAIL";
