...
The repository SHALL provide one canonical validation workflow for code and documentation quality and
one canonical GitHub Pages deployment workflow, and competing or duplicated workflows that serve the
same outcome SHALL not remain active.
...
local and CI environments that lack CUDA hardware.
...
The repository SHALL provide checked-in guidance for Copilot, Claude, and editor/LSP usage that is
specific to HTS, and the default tooling contract SHALL favor lightweight repo-local configuration
over heavyweight integrations that increase maintenance or context cost without clear value.
...
If versioned git hooks are provided, they SHALL run fast checks that reinforce repository standards
without rewriting unrelated files or blocking work on unsupported environments.
