...
void DependencyManager::block_dependents(TaskId id) {
  ...
  while (!queue.empty()) {
    ...
    if (blocked_.count(current) || completed_.count(current) ||
        failed_.count(current)) {
      continue;
    }
...
std::vector<TaskId> DependencyManager::get_ready_tasks() const {
  ...
  for (const auto &[id, count] : pending_deps_) {
    if (count == 0 && !completed_.count(id) && !failed_.count(id) &&
        !blocked_.count(id)) {
      ready.push_back(id);
    }
  }
...
void DependencyManager::reset() { init_pending_counts(); }
