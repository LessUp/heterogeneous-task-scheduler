...
TaskBarrier::TaskBarrier(const std::string &name, TaskGraph &graph) : name_(name), graph_(graph) {
...
void TaskBarrier::wait() {
    ...
    cv_.wait(lock, [this] { return completed_count_.load() >= predecessors_.size(); });
}
