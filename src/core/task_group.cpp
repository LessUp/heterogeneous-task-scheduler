...
TaskGroup::TaskGroup(const std::string &name, TaskGraph &graph)
...
void TaskGroup::add_task(TaskPtr task) { tasks_.push_back(task); }
...
void TaskGroup::depends_on(TaskPtr task) { depends_on(task->id()); }
...
void TaskGroup::then(TaskPtr task) { then(task->id()); }
