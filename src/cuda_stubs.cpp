...
void* MemoryPool::allocate(size_t bytes) {
    ...
    // Find or create a block of the right size
    auto it = free_lists_.find(block_size);
    ...
    if (allow_growth_ && expand_pool(block_size)) {
        // Recursively call allocate after expansion
        std::lock_guard<std::mutex> lock2(mutex_);
        auto it2 = free_lists_.find(block_size);
        if (it2 != free_lists_.end() && !it2->second.empty()) {
            MemoryBlock block = it2->second.front();
...
void MemoryPool::update_fragmentation() const {
    ...
    stats_.fragmentation_ratio = 1.0 - (static_cast<double>(stats_.used_bytes) / stats_.total_bytes);
}
...
void ExecutionEngine::cpu_worker_loop() {
    while (true) {
        std::function<void()> task;
        {
            ...
            if (stop_flag_ && cpu_task_queue_.empty()) return;
...
std::future<void> ExecutionEngine::execute_task(std::shared_ptr<Task> task, DeviceType device) {
    ...
    if (device == DeviceType::CPU) {
        ...
        active_gpu_tasks_++;
        auto gpu_future = std::async(std::launch::async, [this, task, p = std::move(promise)]() mutable {
...
Scheduler::Scheduler(const SchedulerConfig& config)
    ...
    memory_pool_ = std::make_unique<MemoryPool>(config.memory_pool_size, config.allow_memory_growth);
    engine_ = std::make_unique<ExecutionEngine>(*memory_pool_, config.cpu_thread_count, config.gpu_stream_count);
}

Scheduler::~Scheduler() {
    if (executing_) engine_->wait_all();
}
...
void Scheduler::execute_internal() {
    ...
    executing_ = true;
    if (profiling_enabled_) profiler_.start();
    ...
    for (TaskId id : ready) {
        ...
        if (task) {
            ...
            if (profiling_enabled_) profiler_.record_task_start(id, task->name(), device);
    ...
    while (!task_futures.empty()) {
            ...
            if (future.wait_for(std::chrono::milliseconds(1)) == std::future_status::ready) {
                ...
                try {
                    ...
                    if (task) {
                        ...
                        if (profiling_enabled_) profiler_.record_task_end(id, TaskState::Completed);
                    }
                    ...
                    if (task) {
                        ...
                        if (profiling_enabled_) profiler_.record_task_end(id, TaskState::Failed);
                    }
                }
                completed.push_back(id);
            }
        }
        for (TaskId id : completed) task_futures.erase(id);
        ...
        if (task_futures.empty()) break;
        std::this_thread::sleep_for(std::chrono::microseconds(100));
    }
    timeline_.graph_end = std::chrono::high_resolution_clock::now();
    stats_.total_time = std::chrono::duration_cast<std::chrono::nanoseconds>(timeline_.graph_end - timeline_.graph_start);
    ...
    if (profiling_enabled_) profiler_.stop();
    executing_ = false;
}
...
void Scheduler::on_task_failed(TaskId id, const std::string& error) {
    ...
    if (error_callback_) error_callback_(id, error);
}
...
std::string Scheduler::generate_timeline_json() const {
    ...
    for (size_t i = 0; i < timeline_.events.size(); ++i) {
        ...
        oss << "    }";
        if (i < timeline_.events.size() - 1) oss << ",";
        oss << "\n";
    }
    oss << "  ]\n";
    oss << "}\n";
