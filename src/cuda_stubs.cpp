...
ExecutionEngine::ExecutionEngine(MemoryPool &memory_pool, size_t cpu_threads,
                                 size_t gpu_streams)
    : memory_pool_(memory_pool), stream_manager_(gpu_streams),
...
void ExecutionEngine::cpu_worker_loop() {
  while (true) {
    std::function<void()> task;
    {
      ...
      cpu_cv_.wait(lock,
                   [this]() { return stop_flag_ || !cpu_task_queue_.empty(); });
...
std::future<void> ExecutionEngine::execute_task(std::shared_ptr<Task> task,
                                                DeviceType device) {
  ...
  if (device == DeviceType::CPU) {
    ...
    auto gpu_future = std::async(
        std::launch::async, [this, task, p = std::move(promise)]() mutable {
...
TaskContext &ExecutionEngine::get_context(TaskId id) {
  ...
  if (it == contexts_.end()) {
    auto [inserted, _] =
        contexts_.emplace(id, std::make_unique<TaskContext>(&memory_pool_));
...
void ExecutionEngine::wait_all() {
  {
    std::unique_lock<std::mutex> lock(cpu_mutex_);
    cpu_cv_.wait(lock, [this]() {
      return cpu_task_queue_.empty() && active_cpu_tasks_ == 0;
    });
  }
  {
    std::lock_guard<std::mutex> lock(gpu_mutex_);
...
double ExecutionEngine::get_gpu_load() const {
  return static_cast<double>(active_gpu_tasks_.load()) /
