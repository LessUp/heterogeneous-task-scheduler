...
MemoryPool::MemoryPool(size_t initial_size_bytes, bool allow_growth)
    : pool_size_(round_up_power_of_two(initial_size_bytes)),
      base_pool_size_(pool_size_), allow_growth_(allow_growth) {
...
void *MemoryPool::allocate(size_t bytes) {
  ...
  for (auto it = free_lists_.lower_bound(required_size);
       it != free_lists_.end(); ++it) {
    if (!it->second.empty()) {
  ...
  if (!allow_growth_) {
    cv_.wait(lock, [this, required_size]() {
      for (auto it = free_lists_.lower_bound(required_size);
           it != free_lists_.end(); ++it) {
        if (!it->second.empty())
          return true;
    ...
    while (merged) {
      ...
      for (auto it1 = blocks.begin(); it1 != blocks.end(); ++it1) {
        for (auto it2 = std::next(it1); it2 != blocks.end(); ++it2) {
          ...
          if (higher_offset - lower_offset == size &&
              (lower_offset % (2 * size)) == 0) {
            // Merge buddies
...
void MemoryPool::update_fragmentation() {
  ...
  if (total_free == 0) {
    ...
    stats_.fragmentation_ratio =
        1.0 - (static_cast<double>(largest_free) / total_free);
  }
}
