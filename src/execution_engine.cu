...
ExecutionEngine::ExecutionEngine(MemoryPool& memory_pool, 
                                 size_t cpu_threads, 
                                 size_t gpu_streams)
    : memory_pool_(memory_pool)
    , stream_manager_(gpu_streams)
    ...
    while (true) {
        ...
        
        {
            ...
            cpu_cv_.wait(lock, [this]() {
                return stop_flag_ || !cpu_task_queue_.empty();
            });
...
std::future<void> ExecutionEngine::execute_task(std::shared_ptr<Task> task, DeviceType device) {
    ...
    if (device == DeviceType::CPU) {
        ...
        auto gpu_future = std::async(std::launch::async, 
            [this, task, p = std::move(promise)]() mutable {
...
TaskContext& ExecutionEngine::get_context(TaskId id) {
    ...
    if (it == contexts_.end()) {
        auto [inserted, _] = contexts_.emplace(id, 
            std::make_unique<TaskContext>(&memory_pool_));
