...
int main() {
    ...
    add_task->set_gpu_function([&](hts::TaskContext &ctx, cudaStream_t stream) {
        ...
        vector_add_kernel<<<GRID_SIZE, BLOCK_SIZE, 0, stream>>>(d_a.data(), d_b.data(), d_c.data(),
                                                                N);
    });
    ...
    scale_task->set_gpu_function([&](hts::TaskContext &ctx, cudaStream_t stream) {
        ...
        vector_scale_kernel<<<GRID_SIZE, BLOCK_SIZE, 0, stream>>>(d_c.data(), 0.5f, N);
    });
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (std::abs(h_c[i] - expected) > 1e-5) {
            correct = false;
            std::cout << "Mismatch at " << i << ": expected " << expected << ", got " << h_c[i]
                      << std::endl;
        }
    }
