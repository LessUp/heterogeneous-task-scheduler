...
int main() {
  ...
  scale_task->set_gpu_function([&](hts::TaskContext &ctx, cudaStream_t stream) {
    ...
    vector_scale_kernel<<<GRID_SIZE, BLOCK_SIZE, 0, stream>>>(d_c.data(), 0.5f,
                                                              N);
  });
  ...
  std::cout
      << "  CopyToGPU -> VectorAdd -> VectorScale -> ReduceSum -> CopyFromGPU"
      << std::endl;
  std::cout << std::endl;
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    if (std::abs(h_c[i] - expected) > 1e-5) {
      correct = false;
      std::cout << "Mismatch at " << i << ": expected " << expected << ", got "
                << h_c[i] << std::endl;
    }
  }
