...
int main() {
    ...
    for (int chunk = 0; chunk < 4; ++chunk) {
        ...
        task->set_cpu_function([&, chunk](hts::TaskContext &ctx) {
            ...
            std::cout << "[Process " << chunk << "] Processing range [" << start << ", " << end
                      << ")" << std::endl;
    ...
    std::cout << "  Load -> [Process0, Process1, Process2, Process3] -> Aggregate" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "(Sequential would be ~480ms, parallel should be ~250ms)" << std::endl;
