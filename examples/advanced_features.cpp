...
void unreliable_task(TaskContext &ctx, int task_num, double failure_rate) {
    ...
    if (dist(rng) < failure_rate) {
        throw std::runtime_error("Transient failure in task " + std::to_string(task_num));
    }
...
void demo_retry_policies() {
    ...
    for (size_t i = 0; i < 5; ++i) {
        ...
        std::cout << "     Attempt " << i << ": retry=" << (should_retry ? "yes" : "no")
                  << ", delay=" << delay.count() << "ms\n";
    }

    std::cout << "\n  2. Exponential Backoff Policy:\n";
    auto exponential = RetryPolicyFactory::exponential(5, std::chrono::milliseconds(100));
    ...
    auto jittered = RetryPolicyFactory::jittered(5, std::chrono::milliseconds(100));
    for (size_t i = 0; i < 6; ++i) {
    ...
    std::vector<std::string> errors = {"Connection timeout", "Invalid input", "Temporary failure",
                                       "Permission denied"};

    for (const auto &error : errors) {
        bool should_retry = conditional->should_retry(0, error);
        std::cout << "     Error: \"" << error << "\" -> " << (should_retry ? "RETRY" : "NO RETRY")
                  << "\n";
    }
}
...
void demo_resource_limiter() {
    ...
    std::cout << "    Max memory: " << limits.max_memory_bytes / 1024 << " KB\n\n";
    ...
    std::cout << "    Slot 3: " << (slot3 ? "acquired" : "denied") << " (limit reached)\n";
    ...
    std::cout << "    256 KB: " << (alloc3 ? "allocated" : "denied") << " (limit reached)\n";
