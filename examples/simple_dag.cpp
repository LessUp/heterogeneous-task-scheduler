...
int main() {
    ...
    std::cout << "Graph has " << scheduler.graph().size() << " tasks" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "Total time: " << stats.total_time.count() / 1e6 << " ms" << std::endl;
    
    for (const auto& [id, time] : stats.task_times) {
        std::cout << "Task " << id << ": " << time.count() / 1e3 << " us" << std::endl;
    }
