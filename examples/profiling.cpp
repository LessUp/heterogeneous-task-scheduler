...
int main() {
    ...
    final_task->set_cpu_function([](hts::TaskContext& ctx) {
        simulate_work(20);
    });
    ...
    std::cout << "Executing " << scheduler.graph().size() << " tasks..." << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "Completed: " << summary.completed_tasks << "/" << summary.total_tasks << std::endl;
